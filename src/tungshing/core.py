...
def _term_on_day_in_rule_tz(y: int, m: int, d: int, rule_tz: str):
	...
	English: If the given civil date (in rule_tz) has a solar term, return
	  (name, instant@rule_tz[aware], instant@UTC+8[aware]); otherwise None.
	The sxtwl JD2DD returns components in UTC+8, then we convert to rule_tz.
	"""
...
class TungShing:
	...
	def __init__(self, date: datetime = None, *args, **kwargs):
		...
		self._base = _cn.Lunar(dt_naive, *args, **kwargs) # 保持原库参数兼容 / keep cnlunar API
	...
	def _calc_year_gz_strict(self, y: int, dt_rule_naive: datetime) -> str:
		"""
		年柱计算（遵循 GB/T 33661-2017 第 5.1.1 条款）：以立春为年界，秒级切换。
		Year pillar calculation (GB/T 33661-2017 5.1.1): switch at Lìchūn with second-level precision.
		"""
		lichun = None
	...
	def _calc_month_gz_strict(self, y: int, m: int, d: int, dt_rule_naive: datetime) -> str:
		"""
		月柱计算（遵循 GB/T 33661-2017 第 5.1.2 条款）：仅“节”交节后切换（非“中气”）。
		Month pillar calculation (GB/T 33661-2017 5.1.2): switch only at Jié terms (not Zhongqi).
		"""
	...
	def _calc_day_gz_strict(self, hour_rule: int,
		...
		"""
		日柱计算（结合 5.1.3 子时为日界，并作工程化扩展）：
		- 23:00 起算为“次日”的日柱；
		- 23:00–23:59 期间对象整体以前滚的“次日口径”（避免信息割裂）。
		Day pillar calculation (Clause 5.1.3 plus engineering extension):
		- Next day starts from 23:00; forward whole object during 23:00–23:59.
		"""
